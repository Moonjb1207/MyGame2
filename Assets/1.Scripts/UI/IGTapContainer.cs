...
public class IGTapContainer : MonoBehaviour
{
    ...
    private void Awake()
    {
        tapBtns = GetComponentsInChildren<IGTapBtn>(); //하위의 활성화된 오브젝트의 모든 TapBtn 컴포넌트 찾아 배열로 반환
        tapPanels = GetComponentsInChildren<IGTapPanel>(true);//하위의 모든 TapPanel 컴포넌트 배열 찾기 - 비활성화된 오브젝트 포함
        curIdx = 0;
